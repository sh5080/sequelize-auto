...
module.exports = {
  ...
  clearDatabase: function (sequelize, callback, close) {
    ...
    function dropTables() {
      // Oracle needs special handling for dropping all tables
      if (sequelize.getDialect() === 'oracle') {
  ...
  getCreateViewStatement: function (tableName) {
    var statement = {
      ...
      oracle: 'CREATE OR REPLACE VIEW "VHistory" AS SELECT "ARANDOMID" FROM ' + tableName,
    }[this.getTestDialect()];
  ...
  getDropViewStatement: function () {
    var statement = {
      ...
      oracle: 'DROP VIEW IF EXISTS "VHistory";',
    }[this.getTestDialect()];
