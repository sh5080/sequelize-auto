...
describe(helpers.getTestDialectTeaser('sequelize-auto dialects'), function () {
  describe('getForeignKeysQuery', function () {
    ...
    it('oracle', function (done) {
      var query = dialects.oracle.getForeignKeysQuery('mytable_e', 'mydatabase_e');
      expect(query).to.include("table_name = 'MYTABLE_E'");
      expect(query).to.include("owner = 'MYDATABASE_E'");

      query = dialects.oracle.getForeignKeysQuery(null, null);
      expect(query).to.include("table_name = ''");
  ...
  describe('isPrimaryKey', function () {
    ...
    it('oracle', function (done) {
      expect(dialects.oracle.isPrimaryKey(null)).to.be.false;
      expect(dialects.oracle.isPrimaryKey({ some: 'value' })).to.be.false;
      expect(dialects.oracle.isPrimaryKey({ constraint_type: 'R' })).to.be.false;
      expect(dialects.oracle.isPrimaryKey({ constraint_type: 'P' })).to.be.true;
      done();
    });
  });
  ...
  describe('isSerialKey', function () {
    ...
    it('postgres', function (done) {
      ...
      expect(
        dialects.postgres.isSerialKey({ constraint_type: 'PRIMARY KEY', extra: 'nextval(table_seq::regclass)' })
      ).to.be.true;
      done();
    });

    it('oracle', function (done) {
      expect(dialects.oracle.isSerialKey(null)).to.be.false;
      expect(dialects.oracle.isSerialKey({ some: 'value' })).to.be.false;
      // Oracle uses sequences instead of auto_increment
      expect(dialects.oracle.isSerialKey({ extra: 'auto_increment' })).to.be.false;
      done();
    });
  });
});
