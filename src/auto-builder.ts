...
import { Field, IndexSpec, Table, TableData, TableResult } from './types';
...
export class AutoBuilder {
  ...
  private processTables(tableResult: TableResult[]) {
    // tables is an array of either three things:
    // * objects with two properties table_name/TABLE_NAME and table_schema/TABLE_SCHEMA
    // * objects with two properties tableName and tableSchema
    // * objects with a single name property
    // The first happens for dialects which support schemas (e.g. mssql, postgres, oracle).
    // The second happens for dialects which do not support schemas (e.g. sqlite).

    let tables = _.map(tableResult, (t: TableResult) => {
      return {
